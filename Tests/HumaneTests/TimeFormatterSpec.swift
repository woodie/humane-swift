    ...
    override class func spec() {
        ...
        describe("TimeFormatter#string at the approximate-distance bucket table boundaries, without approximate") {
            ...
            it("44:29 stays 44 minutes") {
                let result: String = formatter.string(for: base.addingTimeInterval(-(44 * 60 + 29)), relativeTo: base)
                expect(result).to(equal("44 minutes ago"))
            }

            it("44:30 rounds up to 1 hour") {
                let result: String = formatter.string(for: base.addingTimeInterval(-(44 * 60 + 30)), relativeTo: base)
                expect(result).to(equal("1 hour ago"))
            }

            it("89:29 stays 1 hour") {
                let result: String = formatter.string(for: base.addingTimeInterval(-(89 * 60 + 29)), relativeTo: base)
                expect(result).to(equal("1 hour ago"))
            }

            it("89:30 rounds up to 2 hours") {
                let result: String = formatter.string(for: base.addingTimeInterval(-(89 * 60 + 30)), relativeTo: base)
                expect(result).to(equal("2 hours ago"))
            }

            it("23:59:29 stays 24 hours") {
                let result: String = formatter.string(for: base.addingTimeInterval(-(23 * 3600 + 59 * 60 + 29)), relativeTo: base)
                expect(result).to(equal("24 hours ago"))
            }

            it("23:59:30 rounds up to 1 day") {
                let result: String = formatter.string(for: base.addingTimeInterval(-(23 * 3600 + 59 * 60 + 30)), relativeTo: base)
